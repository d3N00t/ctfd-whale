...
function loadInfo() {
	...
	console.log(response);
		...
		if (response.user_access.indexOf(":8080") ==-1){
            $('#whale-panel').html(
                '<div class="card" style="width: 100%;">' +
                '<div class="card-body">' +
                '<h5 class="card-title">Instance Info</h5>' +
                '<h6 class="card-subtitle mb-2 text-muted" id="whale-challenge-count-down">剩余时间a: ' + response.remaining_time + 's</h6>' +
                '<h6 class="card-subtitle mb-2 text-muted" style="display: none;">Lan Domain: ' + response.lan_domain + '</h6>' +
                '<p class="card-text"><a href="http://'+response.user_access+'" target="_blank">' + response.user_access + '</a></p>' +
                '<button type="button" class="btn btn-danger card-link" id="whale-button-destroy" onclick="CTFd._internal.challenge.destroy()">销毁靶机</button>' +
                '<button type="button" class="btn btn-success card-link" id="whale-button-renew" onclick="CTFd._internal.challenge.renew()">延迟靶机</button>' +
                '</div>' +
                '</div>'
            );
		}else{
		$('#whale-panel').html(
                ...
                '<h6 class="card-subtitle mb-2 text-muted" id="whale-challenge-count-down">剩余时间a: ' + response.remaining_time + 's</h6>' +
                '<h6 class="card-subtitle mb-2 text-muted" style="display: none;">Lan Domain: ' + response.lan_domain + '</h6>' +
                '<p class="card-text"><a href="'+response.user_access+'" target="_blank">' + response.user_access + '</a></p>' +
                '<button type="button" class="btn btn-danger card-link" id="whale-button-destroy" onclick="CTFd._internal.challenge.destroy()">销毁靶机</button>' +
                '<button type="button" class="btn btn-success card-link" id="whale-button-renew" onclick="CTFd._internal.challenge.renew()">延迟靶机</button>' +
                '</div>' +
                '</div>'
